...
質問に答えるだけで、あなた好みのコーヒーの**品種・産地・フレーバー・飲み方・焙煎度・精製方法**がわかる診断 Web アプリです。

公開 URL: https://tomato3713.github.io/coffee-type-diagnosis/

## 特徴

- **コーヒーの知識は不要**。「お茶を選ぶなら？」「デザートを選ぶなら？」といった日常の嗜好を聞く質問（基本12問＋フレーバー深掘り5〜6問＋焙煎度3問＋精製方法3問）に答えるだけ
- 結果は **16タイプ × 焙煎度8段階 × 精製方法7種** の組み合わせ。品種・産地・おすすめの飲み方を提案し、焙煎度は8段階のビジュアルスケールで表示
...
2. **フレーバー深掘り**: フルーティ／ナッティの判定結果で質問が分岐（フルーティ5問・ナッティ6問）。各回答が SCA フレーバーホイールの中分類（実際の中分類にほぼ準拠した18種）に投票し、得票のあった分類をすべて（多い順に）あなたの好みのフレーバーとして表示する
3. **焙煎度3問**: 深煎り側の選択肢が 2 進の桁（4/2/1）の重みを持ち、合計 +1 でライトロースト〜イタリアンローストの8段階が決まる
4. **精製方法3問**: 3問の回答ビット（クリーン↔芳醇 / 甘さ / 定番↔挑戦）の組み合わせ8通りを、ウォッシュト・ナチュラル・ハニー・ホワイトハニー・イエローハニー・スマトラ式・アナエロビックファーメンテーションの7種に対応づける

焙煎度と精製方法は16タイプとは独立に判定されるため、結果は全組み合わせ（16 × 8 × 7 = 896通り）になりえます。
...
├── types.ts            # 共有型定義
├── data/
│   ├── questions.ts    # 基本12問 + フレーバー深掘り + 焙煎度 + 精製方法の質問
│   └── results.ts      # 16タイプ・フレーバー分類・焙煎度・精製方法の定義
